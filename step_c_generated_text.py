# Anslysis #2: This part uses Markov model to generate new film reviews based on the previous scraped film reviews
...
def generate_markov_model(text, order=1):
    """Generate a Markov model of a text file based on the conditional probabilities of each word given its preceding words.

    Args:
        text (str): The input text file as a string.
        order (int, optional): The order of the Markov model, i.e., the number of preceding words to consider for each word. Defaults to 1.

    Returns:
        dict: The Markov model as a dictionary of dictionaries, where the outer keys are tuples of `order` words, and the inner keys are words that follow the corresponding prefix in the text, and the values are the transition probabilities.
...
def generate_text(model, length=100):
    ...
    Args:
        model (dict): The Markov model as a dictionary of dictionaries, where the outer keys are tuples of `order` words, and the inner keys are words that follow the corresponding prefix in the text, and the values are the transition probabilities.
        length (int, optional): The length of the generated text in number of words. Defaults to 100.
...
def main():
    ...
    with open('stranger_things_reviews.txt', 'r', encoding='utf-8') as f:
        text = f.read()
    ...
    with open('vampire_diaries_reviews.txt', 'r', encoding='utf-8') as f:
        text = f.read()
