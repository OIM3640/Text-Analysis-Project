...
def find_most_pos(sentence_list):
    ...
    for i in range(len(phrases_analyzed.keys())):
        if current_best < phrases_analyzed['Phrase '+str(i+1)]['pos']:
            ...
            phrase_number = i+1
    print(f"The most positive phrase out of this list was Phrase {phrase_number} with a score of {current_best}")
    ...
    if answer == 'pos' or answer == 'neu' or answer == 'neg' or answer == 'compound':
        ...
        for i in range(len(phrases_analyzed.keys())):
            if current_best < phrases_analyzed['Phrase '+str(i+1)][answer]:
                ...
                phrase_number = i+1 #Phrase numbers begin at 1 so they are 1 higher than their index
        if answer == 'compound':
            print(f"The phrase with the highest compound score is Phrase {phrase_number} with a score of {current_best}")
        else:
            print(f"The most {answer} phrase is Phrase {phrase_number} with a score of {current_best}")
    else:
