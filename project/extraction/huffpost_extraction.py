"""
HUFFPOST ARTICLES SCRAPE
"""

import newspaper
import json

def build_huffpost_source():

    huffpost_paper = newspaper.build(
    ...
    print(f"Found {len(huffpost_paper.articles)} articles.")

    articles_collected = []

    i = 1
    for article in huffpost_paper.articles[:50]:
        try:
            article.download()
            article.parse()
...
def save_to_json(data):
    with open("project/data/huffpost_articles.json", "w", encoding = "utf-8") as file:
    ...
    print(f"Saved {len(data)} articles to data/huffpost_articles.json")

if __name__ == "__main__":
    articles = build_huffpost_source()
