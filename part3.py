...
def main():
    ...
    print(
        "Summary for 'Entrepreneurship':", compute_summary(entrepreneurship_frequency)
    )

    # Specific words' frequencies
    target_words = ["entrepreneurship", "entrepreneur", "entrepreneurial"]
    specific_frequencies = {
        word: babson_frequency.get(word, 0) for word in target_words
    }
    ...
    print(
        "Sentiment Analysis for 'Entrepreneurship':",
        analyze_sentiment(entrepreneurship_text),
    )

    # Text similarity
    print(
        "\nSimilarity between 'Babson College' and 'Entrepreneurship':",
        compute_text_similarity(babson_text, entrepreneurship_text),
    )
